...
arp_packet_t *process_arp(raw_packet_t raw_packet) {
    ...
    arp_packet_t *a = (arp_packet_t *) raw_packet;

    a ->eth_hdr.frame_type = ntohs(a->eth_hdr.frame_type);
    a->arp_hdr.htype = ntohs(a->arp_hdr.htype);
    a->arp_hdr.ptype = ntohs(a->arp_hdr.ptype);
    a->arp_hdr.op = ntohs(a->arp_hdr.op);
...
icmp_packet_t *process_icmp(ip_packet_t *ip){
    icmp_packet_t *icmp_p = (icmp_packet_t *)ip;

    icmp_p->ip.eth_hdr.frame_type = ntohs(icmp_p->ip.eth_hdr.frame_type);
    icmp_p->ip.ip_hdr.identification = ntohs(icmp_p->ip.ip_hdr.identification);
    //Converting total length causes issues with calculating the payload size
    // icmp_p->ip.ip_hdr.total_length = ntohs(icmp_p->ip.ip_hdr.total_length);
    icmp_p->ip.ip_hdr.flags_and_fragment_offset = ntohs(icmp_p->ip.ip_hdr.flags_and_fragment_offset);
    icmp_p->ip.ip_hdr.header_checksum = ntohs(icmp_p->ip.ip_hdr.header_checksum);
    icmp_p->icmp_hdr.checksum = ntohs(icmp_p->icmp_hdr.checksum);
...
icmp_echo_packet_t *process_icmp_echo(icmp_packet_t *icmp){
    icmp_echo_packet_t *icmp_e = (icmp_echo_packet_t *)icmp;
    icmp_e->icmp_echo_hdr.id = ntohs(icmp_e->icmp_echo_hdr.id);
    icmp_e->icmp_echo_hdr.sequence = ntohs(icmp_e->icmp_echo_hdr.sequence);
    icmp_e->icmp_echo_hdr.timestamp = ntohl(icmp_e->icmp_echo_hdr.timestamp);
    icmp_e->icmp_echo_hdr.timestamp_ms = ntohl(icmp_e->icmp_echo_hdr.timestamp_ms);
